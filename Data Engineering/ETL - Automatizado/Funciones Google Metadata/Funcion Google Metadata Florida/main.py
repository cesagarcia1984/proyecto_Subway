...
def limpiar_df(df):
    ...
    try:
        #================= ETL ================================
        # Filtrar por estado
        patron_pa = r'\b(?:FL|Florida)\b'
        # Filtra el DataFrame para incluir solo filas donde 'address' contiene la cadena 'FL' en cualquier forma escrita
        google_metadata_florida = df[df['address'].str.contains(patron_pa, case=False, na=False, regex=True)]
        # Filtrar solo negocios de Subway
        google_metadata_florida = google_metadata_florida[google_metadata_florida['name'].str.contains('subway', case=False, na=False) | google_metadata_florida['name'].isnull()]
        # Reindexar el DataFrame después de filtrar
        google_metadata_florida = google_metadata_florida.reset_index(drop=True)
        ...
        subway_rows = google_metadata_florida['name'].isin(valores_a_normalizar)
        # Normaliza los valores específicos en la columna 'name'
        google_metadata_florida.loc[subway_rows, 'name'] = 'Subway'
        # Filtramos solo Subway
        g_metadata_subway_florida = google_metadata_florida[google_metadata_florida['name'] == 'Subway']
        # Eliminar duplicados en gmap_id
        g_metadata_subway_florida = g_metadata_subway_florida.drop_duplicates(subset='gmap_id', keep='first').reset_index(drop=True)
        # Normalizamos fechas largas 
        g_metadata_subway_florida['address'] = g_metadata_subway_florida['address'].apply(lambda x: x.rsplit(', United States', 1)[0].strip() if x.endswith(', United States') else x)
        # Separa la columna 'address para extraer datos de la ubicación
        g_metadata_subway_florida = g_metadata_subway_florida.copy()
        g_metadata_subway_florida['address'] = g_metadata_subway_florida['address'].str.split(',').str[1:].str.join(',').str.strip()
        # Extrae datos de la ubicación
        g_metadata_subway_florida['city'] = g_metadata_subway_florida['address'].str.split(',').str[1]
        g_metadata_subway_florida['state_PostalCode'] = g_metadata_subway_florida['address'].str.rsplit(',', n=1).str.get(-1).str.strip()
        # Dividir la cadena en código postal y resto
        g_metadata_subway_florida[['short_state', 'postal_code']] = g_metadata_subway_florida['state_PostalCode'].str.rsplit(n=1, expand=True)
        # Filtrar dígitos para obtener el código postal
        g_metadata_subway_florida['postal_code'] = g_metadata_subway_florida['postal_code'].apply(lambda x: ''.join(filter(str.isdigit, str(x))))
        # Filtrar solo el estado deseado
        g_metadata_subway_florida = g_metadata_subway_florida[g_metadata_subway_florida['short_state'] == 'FL']
        # Eliminar columnas innecesarias
        g_metadata_subway_florida.drop(columns=['state','url','description','hours','address','state_PostalCode', 'price'], inplace=True)
        # Crea la nueva columna 'state' con el valor 'Florida'
        g_metadata_subway_florida['state'] = 'Florida'
        
        #========================================================
        g_metadata_subway_florida.to_csv("tb_google_metadata.csv", index = False) # Nombre de la tabla que se creará en BigQuery

        return g_metadata_subway_florida
...
def captura_evento(event, context):
    ...
    try:
        ...
        if '/' in file_path:
            ...
            if main_folder == "google_metadata_florida": # Nombre carpeta dentro del bucket 
